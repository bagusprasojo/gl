...
class CashBankAddonTests(TestCase):
    ...
    def test_cashbank_account_stores_bank_metadata(self):
        self.activate_module()

        cash_account = create_cashbank_account(
            self.company,
            'Bank Operasional',
            self.bank,
        ...
        self.assertEqual(cash_account.account_kind, CashBankAccount.BANK)
        self.assertEqual(cash_account.bank_name, 'Bank BCA')
        self.assertEqual(cash_account.account_number, '1234567890')
        self.assertEqual(cash_account.account_holder, 'Cashbank UMKM')
        self.assertEqual(cash_account.notes, 'Rekening operasional utama')
        self.assertEqual(str(cash_account), 'Bank Operasional (1234567890)')

    def test_cashbank_account_number_must_be_unique_per_company_when_filled(self):
        self.activate_module()
        create_cashbank_account(
            self.company,
            'Bank Operasional',
